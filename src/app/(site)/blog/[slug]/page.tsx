...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!post) return {};
  return buildMetadata({ title: post.seoTitle || post.title, description: post.seoDescription || post.excerpt, path: `/blog/${post.slug}`, type: "article", image: post.coverImage ?? undefined, publishedTime: post.publishedAt?.toISOString(), modifiedTime: post.updatedAt.toISOString(), keywords: splitList(post.tags) });
}
...
export default async function PostPage({ params }: Props) {
  ...
  return (
    <>
      <JsonLd data={graph(articleSchema({ path, title: post.title, description: post.excerpt, image: post.coverImage ?? undefined, datePublished: (post.publishedAt ?? post.createdAt).toISOString(), dateModified: post.updatedAt.toISOString(), author: post.author, tags }), breadcrumbSchema(crumbs), faqSchema(faqs))} />
      <article>
          ...
          <Container>
            ...
            <div className="mt-8 max-w-3xl" data-reveal>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-5 text-[13.5px] text-slate">
                <span className="inline-flex items-center gap-2"><span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-gold-gradient font-display text-xs font-semibold text-ink">BB</span> {post.author}</span>
                <span className="inline-flex items-center gap-1.5"><Calendar size={14} /> {formatDate(post.publishedAt ?? post.createdAt)}</span>
            ...
            <div className="lg:col-span-8">
              ...
              {tags.length > 0 && (
                <div className="mt-10 flex flex-wrap items-center gap-2 text-[13px]">
                  ...
                  {tags.map((t) => (
                    <Link key={t} href={`/blog?q=${encodeURIComponent(t)}`} className="rounded-full bg-gold-pale px-3 py-1 font-medium text-gold-deep hover:bg-gold/20">{t}</Link>
                  ))}
                </div>
              )}
              {faqs.length > 0 && (
