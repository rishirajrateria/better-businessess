/**
 * Seed: creates the first admin user (from ADMIN_EMAIL / ADMIN_PASSWORD) and
 * starter content you can edit or delete from /admin. Safe to re-run.
 */
...
async function main() {
  ...
  await prisma.user.upsert({ where: { email }, update: {}, create: { email, name: "Admin", passwordHash: await bcrypt.hash(password, 12) } });
